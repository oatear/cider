...
const languageName = 'css-handlebars';
...
const tokenProvider: any =  {
	...
	tokenizer: {
		...
		selectorbody: [
			[/\{\{/, { token: '@rematch', switchTo: '@handlebarsInSimpleState.selectorbody' }],
			{ include: '@comments' },
		...
		term: [
			[/\{\{/, { token: '@rematch', switchTo: '@handlebarsInSimpleState.term' }],
			{ include: '@comments' },
			[
				'(url-prefix)(\\()',
				['attribute.value', { token: 'delimiter.parenthesis', next: '@urldeclaration' }]
		...
		rulevalue: [
			[/\{\{/, { token: '@rematch', switchTo: '@handlebarsInSimpleState.rulevalue' }],
			{ include: '@comments' },
			{ include: '@strings' },
			{ include: '@term' },
			['!important', 'keyword'],
		...
		urldeclaration: [
			[/\{\{/, { token: '@rematch', switchTo: '@handlebarsInSimpleState.urldeclaration' }],
			{ include: '@strings' },
			['[^)\r\n]+', 'string'],
		...
		strings: [
			[/\{\{/, { token: '@rematch', switchTo: '@handlebarsInSimpleState.strings' }],
...
export default class LanguageCssHandlebars {
    public static registerCssHandlebars() {
